...
//    PromissoryNoteState dummyState = new PromissoryNoteState(Currencies.DOLLARS(0), CHARLIE.getParty(), CHARLIE.getParty());
     ...
     * We need the [PromissoryNoteCordaContract] to not only handle Issues of IOUs but now also Transfers.
     ...
     * Hint:
     * - As with the [Issue] command, add the [Transfer] command within the [PromissoryNoteCordaContract.Commands].
     ...
     *
     *   To match any command that implements [PromissoryNoteCordaContract.Commands]
...
//                return tx.failsWith("Required PromissoryNoteCordaContract.Commands command");
//            });
//            l.transaction(tx -> {
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
...
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.withNewLender(CHARLIE.getParty()));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.verifies();
//            });
//            return null;
//        });
...
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, dummyState);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.withNewLender(CHARLIE.getParty()));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
...
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.withNewLender(CHARLIE.getParty()));
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, dummyState);
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith(" An IOU transfer transaction should only create one output state.");
//            });
//            l.transaction(tx -> {
...
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(1), ALICE.getParty(), BOB.getParty(), Currencies.DOLLARS(0)));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith("Only the lender property may change.");
//            });
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(10), ALICE.getParty(), CHARLIE.getParty(), Currencies.DOLLARS(0)));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith("Only the lender property may change.");
//            });
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(10), ALICE.getParty(), BOB.getParty(), Currencies.DOLLARS(5)));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith("Only the lender property may change.");
//            });
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(10), CHARLIE.getParty(), BOB.getParty(), Currencies.DOLLARS(0)));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.verifies();
//            });
//            return null;
...
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
...
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(10), CHARLIE.getParty(), BOB.getParty(), Currencies.DOLLARS(0)));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), CHARLIE.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith("The borrower, old lender and new lender only must sign an IOU transfer transaction");
//            });
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(10), CHARLIE.getParty(), BOB.getParty(), Currencies.DOLLARS(0)));
//                tx.command(Arrays.asList(CHARLIE.getPublicKey(), BOB.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith("The borrower, old lender and new lender only must sign an IOU transfer transaction");
//            });
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(10), CHARLIE.getParty(), BOB.getParty(), Currencies.DOLLARS(0)));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), MINICORP.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith("The borrower, old lender and new lender only must sign an IOU transfer transaction");
//            });
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
//                tx.output(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou.copy(Currencies.DOLLARS(10), CHARLIE.getParty(), BOB.getParty(), Currencies.DOLLARS(0)));
//                tx.command(Arrays.asList(ALICE.getPublicKey(), BOB.getPublicKey(), CHARLIE.getPublicKey(), MINICORP.getPublicKey()), new PromissoryNoteCordaContract.Commands.Transfer());
//                return tx.failsWith("The borrower, old lender and new lender only must sign an IOU transfer transaction");
//            });
//            l.transaction(tx -> {
//                tx.input(PromissoryNoteCordaContract.IOU_CONTRACT_ID, iou);
