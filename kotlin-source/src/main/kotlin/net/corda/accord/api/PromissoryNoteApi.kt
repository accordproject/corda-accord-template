...
import net.corda.accord.flow.PromissoryNoteSettleFlow
import net.corda.accord.flow.PromissoryNoteTransferFlow
import net.corda.accord.flow.SelfIssueCashFlow
 ...
 */
@Path("iou")
class PromissoryNoteApi(val rpcOps: CordaRPCOps) {
    private val me = rpcOps.nodeInfo().legalIdentities.first().name

    companion object {
        private val logger: Logger = loggerFor<PromissoryNoteApi>()
    }
...
//    }
    ...
    fun transferIOU(@QueryParam(value = "id") id: String,
        ...
        try {
            rpcOps.startFlow(::PromissoryNoteTransferFlow, linearId, newLender).returnValue.get()
    ...
    fun settleIOU(@QueryParam(value = "id") id: String,
        ...
        try {
            rpcOps.startFlow(::PromissoryNoteSettleFlow, linearId, settleAmount).returnValue.get()
