...
import net.corda.accord.ALICE
import net.corda.accord.BOB
import net.corda.accord.CHARLIE
 ...
 * transaction.
 * As with the [PromissoryNoteIssueTests] and [PromissoryNoteTransferTests] uncomment each unit test and run them one at a time. Use the body
...
class PromissoryNoteSettleTests {
    ...
    fun mustIncludeSettleCommand() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.POUNDS))
                input(PromissoryNoteContract.IOU_CONTRACT_ID, inputCash)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, outputCash)
            ...
            transaction {
                ...
                output(PromissoryNoteContract.IOU_CONTRACT_ID, outputCash)
                command(BOB.publicKey, Cash.Commands.Move())
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle()) // Correct Type.
                this.verifies()
            }
        }
    }
    ...
    fun mustBeOneGroupOfIOUs() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iouOne)
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iouTwo)
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iouOne.pay(5.POUNDS))
                input(PromissoryNoteContract.IOU_CONTRACT_ID, inputCash)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, outputCash.ownableState)
                command(BOB.publicKey, Cash.Commands.Move())
    ...
    fun mustHaveOneInputIOU() {
        ...
        ledgerServices.ledger {
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
            ...
            transaction {
                ...
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iouOne.pay(5.POUNDS))
                input(PromissoryNoteContract.IOU_CONTRACT_ID, fivePounds)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, fivePounds.withNewOwner(newOwner = ALICE.party).ownableState)
                command(BOB.publicKey, Cash.Commands.Move())
                this.verifies()
            }
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iouOne)
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
                input(PromissoryNoteContract.IOU_CONTRACT_ID, tenPounds)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, tenPounds.withNewOwner(newOwner = ALICE.party).ownableState)
                command(BOB.publicKey, Cash.Commands.Move())
                this.verifies()
            }
        }
    ...
    fun mustBeCashOutputStatesPresent() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.DOLLARS))
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
                this `fails with` "There must be output cash."
            }
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                input(PromissoryNoteContract.IOU_CONTRACT_ID, cash)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.DOLLARS))
                output(PromissoryNoteContract.IOU_CONTRACT_ID, cashPayment.ownableState)
    ...
    fun mustBeCashOutputStatesWithRecipientAsOwner() {
        ...
        ledgerServices.ledger {
            transaction {
                ...
                input(PromissoryNoteContract.IOU_CONTRACT_ID, cash)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.POUNDS))
                output(PromissoryNoteContract.IOU_CONTRACT_ID, "outputs cash", invalidCashPayment.ownableState)
            ...
            transaction {
                ...
                input(PromissoryNoteContract.IOU_CONTRACT_ID, cash)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.POUNDS))
                output(PromissoryNoteContract.IOU_CONTRACT_ID, validCashPayment.ownableState)
    ...
    fun cashSettlementAmountMustBeLessThanRemainingIOUAmount() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                input(PromissoryNoteContract.IOU_CONTRACT_ID, elevenDollars)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(11.DOLLARS))
                output(PromissoryNoteContract.IOU_CONTRACT_ID, elevenDollars.withNewOwner(newOwner = ALICE.party).ownableState)
            ...
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                input(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.DOLLARS))
                output(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars.withNewOwner(newOwner = ALICE.party).ownableState)
            ...
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                input(PromissoryNoteContract.IOU_CONTRACT_ID, tenDollars)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, tenDollars.withNewOwner(newOwner = ALICE.party).ownableState)
    ...
    fun mustOnlyHaveOutputIOUIfNotFullySettling() {
        ...
        ledgerServices.ledger {
            ...
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, tenDollars)
                input(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(10.DOLLARS))
                output(PromissoryNoteContract.IOU_CONTRACT_ID, tenDollars.withNewOwner(newOwner = ALICE.party).ownableState)
    ...
    fun onlyPaidPropertyMayChange() {
        ...
        ledgerServices.ledger {
            transaction {
                ...
                input(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars.withNewOwner(newOwner = ALICE.party).ownableState)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.copy(borrower = ALICE.party, paid = 5.DOLLARS))
                command(BOB.publicKey, fiveDollars.withNewOwner(newOwner = BOB.party).command)
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
            ...
            transaction {
                ...
                input(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars.withNewOwner(newOwner = ALICE.party).ownableState)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.copy(amount = 0.DOLLARS, paid = 5.DOLLARS))
                command(BOB.publicKey, fiveDollars.withNewOwner(newOwner = BOB.party).command)
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
            ...
            transaction {
                ...
                input(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, fiveDollars.withNewOwner(newOwner = ALICE.party).ownableState)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.copy(lender = CHARLIE.party, paid = 5.DOLLARS))
                command(BOB.publicKey, fiveDollars.withNewOwner(newOwner = BOB.party).command)
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Settle())
    ...
    fun mustBeSignedByAllParticipants() {
        ...
        ledgerServices.ledger {
            transaction {
                ...
                command(BOB.publicKey, cashPayment.command)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.DOLLARS))
                command(listOf(ALICE.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Settle())
            ...
            transaction {
                ...
                command(BOB.publicKey, cashPayment.command)
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou.pay(5.DOLLARS))
                command(BOB.publicKey, PromissoryNoteContract.Commands.Settle())
                failsWith("Both lender and borrower together only must sign IOU settle transaction.")
            }
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, cash)
