 ...
 * As with the [PromissoryNoteIssueTests] uncomment each unit test and run them one at a time. Use the body of the tests and the
...
class PromissoryNoteTransferTests {
    ...
    fun mustHandleMultipleCommandValues() {
        ...
        ledgerServices.ledger {
            transaction {
                output(PromissoryNoteContract::class.java.name, iou)
                command(listOf(ALICE.publicKey, BOB.publicKey), DummyCommand())
                this `fails with` "Required net.corda.accord.contract.PromissoryNoteContract.Commands command"
            }
            transaction {
                output(PromissoryNoteContract::class.java.name, iou)
            ...
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                command(listOf(ALICE.publicKey, BOB.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
                this.verifies()
            }
        }
    }
    ...
    fun mustHaveOneInputAndOneOutput() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                input(PromissoryNoteContract::class.java.name, DummyState())
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                command(listOf(ALICE.publicKey, BOB.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
            ...
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                output(PromissoryNoteContract::class.java.name, DummyState())
                command(listOf(ALICE.publicKey, BOB.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "An IOU transfer transaction should only create one output state."
            }
            transaction {
    ...
    fun onlyTheLenderMayChange() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract::class.java.name, IOUState(10.DOLLARS, ALICE.party, BOB.party))
                output(PromissoryNoteContract::class.java.name, IOUState(1.DOLLARS, ALICE.party, BOB.party))
                command(listOf(ALICE.publicKey, BOB.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "Only the lender property may change."
            }
            transaction {
                input(PromissoryNoteContract::class.java.name, IOUState(10.DOLLARS, ALICE.party, BOB.party))
                output(PromissoryNoteContract::class.java.name, IOUState(10.DOLLARS, ALICE.party, CHARLIE.party))
                command(listOf(ALICE.publicKey, BOB.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "Only the lender property may change."
            }
            transaction {
                input(PromissoryNoteContract::class.java.name, IOUState(10.DOLLARS, ALICE.party, BOB.party, 5.DOLLARS))
                output(PromissoryNoteContract::class.java.name, IOUState(10.DOLLARS, ALICE.party, BOB.party, 10.DOLLARS))
                command(listOf(ALICE.publicKey, BOB.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "Only the lender property may change."
            }
            transaction {
    ...
    fun allParticipantsMustSign() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "The borrower, old lender and new lender only must sign an IOU transfer transaction"
            }
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                command(listOf(ALICE.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "The borrower, old lender and new lender only must sign an IOU transfer transaction"
            }
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                command(listOf(BOB.publicKey, CHARLIE.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "The borrower, old lender and new lender only must sign an IOU transfer transaction"
            }
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                command(listOf(ALICE.publicKey, BOB.publicKey, MINICORP.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "The borrower, old lender and new lender only must sign an IOU transfer transaction"
            }
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
                output(PromissoryNoteContract::class.java.name, iou.withNewLender(CHARLIE.party))
                command(listOf(ALICE.publicKey, BOB.publicKey, CHARLIE.publicKey, MINICORP.publicKey), PromissoryNoteContract.Commands.Transfer())
                this `fails with` "The borrower, old lender and new lender only must sign an IOU transfer transaction"
            }
            transaction {
                input(PromissoryNoteContract::class.java.name, iou)
