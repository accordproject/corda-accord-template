...
import net.corda.accord.DUMMY
import net.corda.accord.MINICORP
...
class PromissoryNoteIssueTests {
     ...
     * - The command should be defined inside [PromissoryNoteContract].
     ...
     *   implements the [CommandData] interface. The [PromissoryNoteContract.Commands.Issue] command itself should be defined inside the [Commands]
    ...
    fun mustIncludeIssueCommand() {
        ...
        ledgerServices.ledger {
            ...
            transaction {
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue()) // Correct type.
                this.verifies()
            }
        }
    }
     ...
     * Hint: use a [requireThat] block with a constraint to inside the [PromissoryNoteContract.verify] function to encapsulate your
     * constraints:
     *
     *     requireThat {
    ...
    fun issueTransactionMustHaveNoInputs() {
        ...
        ledgerServices.ledger {
            transaction {
                input(PromissoryNoteContract.IOU_CONTRACT_ID, DummyState())
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
    ...
    fun issueTransactionMustHaveOneOutput() {
        ...
        ledgerServices.ledger {
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou) // Two outputs fails.
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                this `fails with` "Only one output state should be created when issuing an IOU."
            }
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou) // One output passes.
                this.verifies()
            }
        }
    }
    ...
    fun cannotCreateZeroValueIOUs() {
        ledgerServices.ledger {
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, IOUState(0.POUNDS, ALICE.party, BOB.party)) // Zero amount fails.
                this `fails with` "A newly issued IOU must have a positive amount."
            }
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, IOUState(100.SWISS_FRANCS, ALICE.party, BOB.party))
                this.verifies()
            }
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, IOUState(1.POUNDS, ALICE.party, BOB.party))
                this.verifies()
            }
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, IOUState(10.DOLLARS, ALICE.party, BOB.party))
                this.verifies()
            }
        }
    }
    ...
    fun lenderAndBorrowerCannotBeTheSame() {
        ...
        ledgerServices.ledger {
            transaction {
                command(listOf(ALICE.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, borrowerIsLenderIou)
    ...
    fun lenderAndBorrowerMustSignIssueTransaction() {
        ...
        ledgerServices.ledger {
            transaction {
                command(DUMMY.publicKey, PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                this `fails with` "Both lender and borrower together only may sign IOU issue transaction."
            }
            transaction {
                command(ALICE.publicKey, PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                this `fails with` "Both lender and borrower together only may sign IOU issue transaction."
            }
            transaction {
                command(BOB.publicKey, PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                this `fails with` "Both lender and borrower together only may sign IOU issue transaction."
            }
            transaction {
                command(listOf(BOB.publicKey, BOB.publicKey, BOB.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                this `fails with` "Both lender and borrower together only may sign IOU issue transaction."
            }
            transaction {
                command(listOf(BOB.publicKey, BOB.publicKey, MINICORP.publicKey, ALICE.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                this `fails with` "Both lender and borrower together only may sign IOU issue transaction."
            }
            transaction {
                command(listOf(BOB.publicKey, BOB.publicKey, BOB.publicKey, ALICE.publicKey), PromissoryNoteContract.Commands.Issue())
                output(PromissoryNoteContract.IOU_CONTRACT_ID, iou)
                this.verifies()
            }
            transaction {
