...
class PromissoryNoteSettleFlowTests {
    ...
    fun flowReturnsCorrectlyFormedPartiallySignedTransaction() {
        val stx = issueIou(PromissoryNoteState(10.POUNDS, b.info.chooseIdentityAndCert().party, a.info.chooseIdentityAndCert().party))
        issueCash(5.POUNDS)
        val inputIou = stx.tx.outputs.single().data as PromissoryNoteState
        val flow = PromissoryNoteSettleFlow(inputIou.linearId, 5.POUNDS)
        val future = a.startFlow(flow)
        mockNetwork.runNetwork()
        ...
        a.transaction {
            ...
            val outputIou = ledgerTx.outputs.map { it.data }.filterIsInstance<PromissoryNoteState>().single()
            assertEquals(
                    outputIou,
            ...
            val command = ledgerTx.commands.requireSingleCommand<PromissoryNoteContract.Commands>()
            assert(command.value == PromissoryNoteContract.Commands.Settle())
