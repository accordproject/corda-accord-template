...
class PromissoryNoteTransferFlowTests {
    ...
    /**
     * Issue an PromissoryNote on the ledger, we need to do this before we can transfer one.
     */
    private fun issueIou(iou: PromissoryNoteState): SignedTransaction {
     ...
     * - Use the [UniqueIdentifier] which is passed into the flow to retrieve the correct [PromissoryNoteState].
     ...
     *   [PublicKey]s to the Command's signers list. We can get the signers from the input PromissoryNote and the new PromissoryNote you
    ...
    fun flowReturnsCorrectlyFormedPartiallySignedTransaction() {
        ...
        val stx = issueIou(PromissoryNoteState(10.POUNDS, lender, borrower))
        val inputIou = stx.tx.outputs.single().data as PromissoryNoteState
        val flow = PromissoryNoteTransferFlow(inputIou.linearId, c.info.chooseIdentityAndCert().party)
        val future = a.startFlow(flow)
        mockNetwork.runNetwork()
        ...
        val outputIou = ptx.tx.outputs.single().data as PromissoryNoteState
        println("Output state: $outputIou")
        val command = ptx.tx.commands.single()
        assert(command.value == PromissoryNoteContract.Commands.Transfer())
    ...
    fun iouCannotBeTransferredToSameParty() {
        ...
        val stx = issueIou(PromissoryNoteState(10.POUNDS, lender, borrower))
        val inputIou = stx.tx.outputs.single().data as PromissoryNoteState
        val flow = PromissoryNoteTransferFlow(inputIou.linearId, lender)
        val future = a.startFlow(flow)
        mockNetwork.runNetwork()
        // Check that we can't transfer an PromissoryNote to ourselves.
